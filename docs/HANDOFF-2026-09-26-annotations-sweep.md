...
- **Mine, caught by the pr-tests sweep (part 1/4, VACUOUS):** once the
  harness defused its own lines, `test_pr_shards.py`'s declared mutation of
  `shown()` no longer turned that file red: a second layer protected the
  same output. Root cause: I added a layer of protection without asking
  which declarations measured the first one. Fixed: both layers read
  `tcheck.DEFUSED`, and the declaration mutates that (BITES).
