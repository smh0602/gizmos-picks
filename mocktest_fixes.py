...
nfl_old = load("nfl_old", f"{OLD}/nfl_OLD.py")
cfb_old = load("cfb_old", f"{OLD}/cfb_OLD.py")
...
ck("🔴 the NFL report shows the ahead_out RuntimeError",
   "ahead_out is CONSTANT ZERO" in REPORTS["nfl"],
   "this is the failure every nfl-logs run has hit since the season "
   "opened 2026-09-09")
ck("🔴 the college report shows the constant-column RuntimeError",
   "depth_rank is CONSTANT None" in REPORTS["ncaaf"],
   "three columns named, one cause")
# 🔴 THE CORRECTION THAT MATTERS: `not yet` is EMPTY on the NFL report.
ck("⚠️ nflverse HAS published — `not yet` is empty on the NFL report",
   "not yet  : []" in REPORTS["nfl"],
   "I told Sam the tab was empty because nflverse had not published. "
   "66 player-weeks REACHED the guard, so the fetch worked and only the "
   "guard blocked the write. That claim was carried from 09-09 and was "
   "wrong by 09-10.")
...
def drive_old(n_ao, n_rows):
    """⛔ The OLD guard, lifted verbatim from the pre-drop file so the
    comparison is against what actually shipped, not a paraphrase."""
...
ck("🔴 OLD CODE flags three constant columns on a ONE-WEEK log",
   len(_old_const) >= 3,
   "the live failure reproduced: %s" % "; ".join(b[:46] for b in _old_const))
