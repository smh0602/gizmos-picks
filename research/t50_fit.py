#!/usr/bin/env python3
"""T50 — pass-snap share vs snap share. Fits and grades EXACTLY the spec
fixed in `claude/owed-tests.md` BEFORE this file existed.

🔴 THE FIX FOR T47'S OWN CONFOUND: EXACTLY ONE THING CHANGES.
`trail8_snap_pct` is REPLACED by `trail8_pass_snap_share`. Everything
else -- rows, split, bars, and the three inputs T47 declared SPENT
(`inj`, `ol_out`, `opp_dl_out`) -- is CARRIED UNCHANGED. ⛔ Dropping them
would be a second change and would forfeit the attribution this test
exists to get.

⚠️ THE DENOMINATOR IS A PROXY, DECLARED BEFORE THE RUN: team pass plays
are the MAX `pass_snaps` among that team's WR/TE/RB in that game, because
linemen are not in the player database. A near-every-down receiver plays
~90-95% of pass snaps, so this understates the denominator and overstates
every share by a roughly constant factor. A constant scaling is absorbed
by the coefficient; the per-game noise is not.
...
RT = json.load(gzip.open(os.path.join(_D, _R), "rt"))["by_player"]

# ── team pass plays per game, the declared PROXY ────────────────────
# ⚠️ max pass_snaps among that team's WR/TE/RB. See the docstring.
_teampass = collections.defaultdict(int)
for _pid, _p in D.items():
    if _p["pos"] not in {"WR", "TE", "RB"}:
        continue
    for _g in _p["g"]:
        _r = RT.get(_pid, {}).get(_g["game_id"])
        if _r:
            k = (_g["game_id"], _g["team"])
            _teampass[k] = max(_teampass[k], _r[1])

def pass_share(pid, g):
    """His share of his team's pass plays. None if the join misses."""
    r = RT.get(pid, {}).get(g["game_id"])
    if not r:
        return None
    den = _teampass.get((g["game_id"], g["team"]), 0)
    return (r[1] / den) if den else None
...
for pid, p in D.items():
    ...
    for i, g in enumerate(gs):
        ...
        # ⛔ THE ONLY CHANGE. Trailing-8 pass-snap share, point-in-time.
        _ps = [pass_share(pid, x) for x in t8]
        _ps = [v for v in _ps if v is not None]
        pshare = (sum(_ps) / len(_ps)) if _ps else None
        oa = opp_allowed(g["o"], p["pos"], g["week"])
        rows.append({
            "pid": pid, "pos": p["pos"], "wk": g["week"], "y": g["rec_yds"],
            "tgt": tgt, "ypt": ypt, "snap": snap, "pshare": pshare,
            "home": g["home"] or 0,
            "oa": oa,
...
_before = len(rows)
rows = [r for r in rows if r["pshare"] is not None]
print(f"route join: kept {len(rows):,} of {_before:,} T47 rows "
      f"({100.0*len(rows)/max(1,_before):.2f}%)")
...
# 🔴 `snap` IS REPLACED BY `pshare`. THAT IS THE WHOLE TEST.
FEAT = ["tgt", "ypt", "pshare", "home",
...
X, y = design(fit)
beta = solve(X, y)
print("\ncoefficients (T50 pre-registered spec, in order):")
